...
데이터베이스 스키마 변경 이력을 안전하게 관리하기 위해 `Alembic`을 도입했습니다. 기존의 서버 기동 시 `create_all`로 테이블을 일괄 생성하던 방식에서 벗어나, 스키마 변경 사항을 버전 단위로 추적하고 롤백할 수 있는 구조로 전환했습니다.

* **동적 설정**: `.env`의 `DATABASE_URL`을 `env.py`에서 동적으로 로드하여 환경별(개발/운영) 마이그레이션을 지원합니다.
* **자동 감지**: `target_metadata` 연결을 통해 모델의 변경 사항을 자동으로 감지하여 마이그레이션 스크립트를 생성합니다.
* **안전성**: 오프라인/온라인 모드 분리 및 환경 변수 검증 로직을 통해 잘못된 마이그레이션 실행을 방지합니다.
...
### 4. Alembic 환경 변수 및 임포트 이슈

* **문제**: Alembic 초기화 시 `env.py`에서 프로젝트 모델을 참조하지 못하거나, `.env` 파일을 읽지 못해 DB 연결에 실패하는 현상 발생.
* **해결**: `env.py` 상단에 `load_dotenv()`를 배치하고, 패키지 구조에 맞게 `from study_room import models`로 임포트 경로를 수정하여 해결. 오프라인 모드에서도 환경 변수를 참조할 수 있도록 로직을 보완.
...
* [x] **Database Migration**: `Alembic` 도입을 통한 스키마 버전 관리 체계 구축.
* [x] **Fail-Fast Strategy**: 서버 기동 시 필수 환경 변수 및 설정 유효성 검사 적용.
* [ ] **Frontend Integration**: React 기반의 직관적인 예약 대시보드 구현.
* [ ] **Deployment**: Docker 컨테이너화 및 Nginx 역방향 프록시 설정.
* [ ] **Test Automation**: Pytest를 이용한 핵심 서비스 로직(예약/취소) 유닛 테스트.

---

## 📝 Retrospective

관계형 데이터베이스의 복잡한 비즈니스 로직을 다루며 **데이터 무결성**과 **트랜잭션 관리**의 중요성을 깊이 이해하게 된 프로젝트였습니다. 특히 예외 상황을 고려한 이중 방어 로직 설계(서비스 레이어 + DB 제약 조건)를 통해 견고한 백엔드 시스템의 필요성을 체감했습니다. 또한 동기 방식에서 비동기 방식(`AsyncSession`)으로 전환하는 과정에서 FastAPI의 `lifespan` 이벤트 핸들러와 비동기 트랜잭션 관리에 대한 이해를 높일 수 있었습니다. 이번 업데이트를 통해 에러 코드 세분화, 로깅 시스템 구축, 트랜잭션 규칙 명문화, 환경 변수 검증 등 **유지보수성과 운영 안정성**을 한 단계 끌어올리는 경험을 쌓을 수 있었습니다. 마지막으로 `Alembic`을 도입하며 단순히 테이블을 생성하는 것을 넘어, **스키마 변경 이력을 체계적으로 버전 관리하는 것**이 장기적인 프로젝트 유지보수에 얼마나 중요한지를 직접 체감할 수 있었습니다.